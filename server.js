...
async function isBetaTesting() {
  try {
    const r = await pool.query('SELECT beta_testing FROM app_settings WHERE id = 1');
    return !!(r.rows[0] && r.rows[0].beta_testing);
  } catch { return false; }
}
...
async function migrate() {
  try {
    ...
    await pool.query(`ALTER TABLE app_settings ADD COLUMN IF NOT EXISTS withdrawal_cooldown_hours INTEGER DEFAULT 0;`);
    await pool.query(`ALTER TABLE app_settings ADD COLUMN IF NOT EXISTS withdrawal_opens_at TIMESTAMPTZ DEFAULT NULL;`);
    await pool.query(`ALTER TABLE app_settings ADD COLUMN IF NOT EXISTS withdrawal_cooldown_enabled BOOLEAN DEFAULT FALSE;`);
    await pool.query(`ALTER TABLE app_settings ADD COLUMN IF NOT EXISTS withdrawal_schedule_enabled BOOLEAN DEFAULT FALSE;`);
    ...
    await pool.query(`ALTER TABLE users ADD COLUMN IF NOT EXISTS og_pass_expires_at TIMESTAMPTZ;`);
    // Backfill: existing OG users get 30 days from bought_at or now
    try {
      await pool.query(`
        UPDATE users SET og_pass_expires_at = COALESCE(og_pass_bought_at, NOW()) + INTERVAL '30 days'
        WHERE og_pass = TRUE AND og_pass_expires_at IS NULL
      `);
    } catch (_) {}
    ...
    // Remove Block Blast missions/achievements (game removed) — every boot
    try {
      await pool.query(`DELETE FROM user_missions WHERE mission_id IN ('play_3_blocks','score_10000') OR mission_id LIKE '%blocks%'`);
      await pool.query(`DELETE FROM missions_catalog WHERE id IN ('play_3_blocks','score_10000') OR metric IN ('blocks_games','blocks_score') OR title ILIKE '%block blast%' OR title ILIKE '%blocks%'`);
      await pool.query(`UPDATE achievements_catalog SET active = FALSE WHERE id = 'blocks_20000' OR trigger_type = 'blocks_high_score' OR title ILIKE '%block blast%'`);
    } catch (e) { console.warn('blocks mission cleanup', e.message); }


    await pool.query(`CREATE TABLE IF NOT EXISTS income_cards (
      id TEXT PRIMARY KEY,
      title TEXT NOT NULL,
      description TEXT DEFAULT '',
      image_url TEXT DEFAULT '',
      sp_per_hour INTEGER NOT NULL DEFAULT 100,
      price_gram NUMERIC DEFAULT 0,
      price_stars INTEGER DEFAULT 0,
      active BOOLEAN DEFAULT TRUE,
      sort_order INTEGER DEFAULT 0
    );`);
    await pool.query(`CREATE TABLE IF NOT EXISTS user_income_cards (
      id SERIAL PRIMARY KEY,
      telegram_id BIGINT NOT NULL,
      card_id TEXT NOT NULL REFERENCES income_cards(id) ON DELETE CASCADE,
      cycle_started_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
      last_claim_at TIMESTAMPTZ,
      active BOOLEAN DEFAULT TRUE,
      card_level INTEGER DEFAULT 1,
      UNIQUE(telegram_id, card_id)
    );`);
    await pool.query(`ALTER TABLE user_income_cards ADD COLUMN IF NOT EXISTS card_level INTEGER DEFAULT 1;`);
    await pool.query(`ALTER TABLE income_cards ADD COLUMN IF NOT EXISTS pay_methods JSONB DEFAULT '["gram","stars"]'::jsonb;`);
    await pool.query(`ALTER TABLE income_cards ADD COLUMN IF NOT EXISTS price_sp INTEGER DEFAULT 0;`);
    await pool.query(`ALTER TABLE income_cards ADD COLUMN IF NOT EXISTS max_level INTEGER DEFAULT 8;`);
    await pool.query(`ALTER TABLE app_settings ADD COLUMN IF NOT EXISTS shop_boosts JSONB DEFAULT '[]'::jsonb;`);
    await pool.query(`ALTER TABLE app_settings ADD COLUMN IF NOT EXISTS beta_testing BOOLEAN DEFAULT FALSE;`);
    ...
    const defaultMissions = [
      ...
      ['play_3_blocks_REMOVED', 'Play 3 Block Blast Games', 'Complete 3 games of ShhhToshi Blocks', 'blocks_games', 3, 300, 40, 2],
      ['score_10000_REMOVED', 'Score 10,000 Points', 'Rack up 10,000 points in Block Blast (single or combined games)', 'blocks_score', 10000, 750, 75, 3],
    ...
    const defaultAchievements = [
      // Easy → Hard (sort_order)
      ['first_tap', 'First Tap', 'Tap for the very first time', 'total_taps', 1, 50, 10, '👆', 1],
      ['reach_silver', 'Reach Silver', 'Grow your Sp balance to Silver rank', 'shhhtoshi', 20000, 300, 30, '🥈', 2],
      ['buy_first_card', 'First Card', 'Buy your first income card', 'cards_owned', 1, 200, 20, '🃏', 3],
      ...
      ['reach_gold', 'Reach Gold', 'Grow your Sp balance to Gold rank', 'shhhtoshi', 50000, 800, 80, '🥇', 6],
      ['upgrade_card_lv3', 'Card Level 3', 'Upgrade any income card to level 3', 'card_level', 3, 400, 40, '⬆️', 7],
      ['buy_og_pass', 'OG Member', 'Purchase the OG Pass', 'og_pass', 1, 1000, 100, '👑', 8],
      ['taps_100000', '100,000 Taps', 'Reach 100,000 lifetime taps', 'total_taps', 100000, 2000, 200, '💥', 9],
      ['cards_5', 'Card Collector', 'Own 5 income cards', 'cards_owned', 5, 1500, 150, '📚', 10],
      ['upgrade_card_lv8', 'Max Card', 'Upgrade any income card to level 8', 'card_level', 8, 3000, 300, '🏆', 11],
      ['reach_legend', 'Reach Legend', 'Climb all the way to Legend rank', 'shhhtoshi', 1500000, 5000, 500, '⭐', 12],
      ['cards_20', 'Card Empire', 'Own 20 income cards', 'cards_owned', 20, 8000, 800, '🏰', 13],
    ];
    for (const [id, title, desc, trigger, threshold, sp, xp, icon, order] of defaultAchievements) {
      await pool.query(
        `INSERT INTO achievements_catalog (id, title, description, trigger_type, threshold, reward_sp, reward_xp, icon, sort_order, active)
         VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9,TRUE)
         ON CONFLICT (id) DO UPDATE SET title=$2, description=$3, trigger_type=$4, threshold=$5, reward_sp=$6, reward_xp=$7, icon=$8, sort_order=$9, active=TRUE`,
        [id, title, desc, trigger, threshold, sp, xp, icon, order]
      );
    }
    // Disable removed systems
    await pool.query(`UPDATE achievements_catalog SET active = FALSE WHERE id IN ('level_25','blocks_20000')`);
...
async function fetchAndCachePhoto(userId, force = false) {
  try {
    const dir = path.join(__dirname, 'public', 'photos');
    if (!fs.existsSync(dir)) fs.mkdirSync(dir, { recursive: true });
    const localName = String(userId) + '.jpg';
    const localPath = path.join(dir, localName);
    const publicUrl = '/photos/' + localName;
    ...
    if (!force && fs.existsSync(localPath)) {
      ...
      if (ageMs < 7 * 24 * 60 * 60 * 1000) {
        await pool.query('UPDATE users SET telegram_photo_url = $1 WHERE telegram_id = $2', [publicUrl, userId]);
        return publicUrl;
      }
    }
    ...
    console.warn('fetchAndCachePhoto', e.message);
    return null;
  }
}

async function ensurePhotosForUsers(ids) {
  const list = [...new Set((ids || []).map(Number).filter(Boolean))].slice(0, 30);
  await Promise.all(list.map(id => fetchAndCachePhoto(id).catch(() => null)));
}
...
app.post('/api/auth', authLimiter, authMiddleware, async (req, res) => {
  ...
  try {
    const adminIds = await getAdminIds();
    const isAdmin = adminIds.includes(parseInt(id));
    // Beta testing: no user rows written for non-admins — ephemeral session only
    if (await isBetaTesting() && !isAdmin) {
      return res.json({
        betaTesting: true,
        isAdmin: false,
        user: {
    ...
    // Prefer WebApp init photo_url (stable-ish CDN). Otherwise download via Bot API into /photos/{id}.jpg
    if (photo_url && String(photo_url).startsWith('http')) {
      await pool.query('UPDATE users SET telegram_photo_url = $1 WHERE telegram_id = $2', [photo_url, id]);
      user = { ...user, telegram_photo_url: photo_url };
    }
    const needFetch = !user.telegram_photo_url || String(user.telegram_photo_url).includes('api.telegram.org/file/');
    if (needFetch) {
      const photoUrl = await Promise.race([
        fetchAndCachePhoto(id, true),
        new Promise(resolve => setTimeout(() => resolve(null), 3500))
      ]);
      if (photoUrl) user = { ...user, telegram_photo_url: photoUrl };
    }

    res.json({ user, isAdmin: isAdmin || adminIds.includes(parseInt(id)), betaTesting: await isBetaTesting() });
  } catch (err) {
...
app.get(['/api/auth/me', '/api/user/me'], authMiddleware, async (req, res) => {
  try {
    ...
    let user = r.rows[0];
    if (user.banned) return res.status(403).json({ error: 'banned' });
    const needPhoto = !user.telegram_photo_url || String(user.telegram_photo_url).includes('api.telegram.org/file/');
    if (needPhoto) {
      const photoUrl = await Promise.race([
        fetchAndCachePhoto(req.tgUser.id, true),
...
app.post(['/api/user/update', '/api/user/sync', '/api/user/wallet'], authMiddleware, async (req, res) => {
  ...
  try {
    if (await isBetaTesting()) {
      const adminIds = await getAdminIds();
      if (!adminIds.includes(parseInt(req.tgUser.id))) {
        return res.json({ ok: true, betaTesting: true, skipped: true });
      }
    }
    const fields = [];
    const vals = [];
    ...
    if (validDelta) {
      ...
      try { await checkProgressAchievements(req.tgUser.id); } catch (_) {}
    }
...
// OG Pass: +20% on task / mission / ad / achievement Sp rewards
async function userHasActiveOg(tgId) {
  try {
    const r = await pool.query('SELECT og_pass, og_pass_expires_at FROM users WHERE telegram_id = $1', [tgId]);
    if (!r.rows[0]?.og_pass) return false;
    const exp = r.rows[0].og_pass_expires_at;
    if (exp && new Date(exp).getTime() < Date.now()) {
      await pool.query('UPDATE users SET og_pass = FALSE WHERE telegram_id = $1', [tgId]).catch(() => {});
      return false;
    }
    return true;
...
async function withOgTaskBonus(tgId, amount) {
  let n = Math.floor(Number(amount) || 0);
  if (n <= 0) return 0;
  try {
    if (await userHasActiveOg(tgId)) n = Math.floor(n * 1.2);
  } catch (_) {}
  return n;
}
...
app.post('/api/task/complete', taskLimiter, authMiddleware, async (req, res) => {
  ...
  try {
    ...
    // OG Pass: +20% on ALL task rewards (regular + ads via this route)
    let finalReward = await withOgTaskBonus(tgId, reward);
    ...
    res.json({ user: result.rows[0], newCosmetics, reward: finalReward });
  } catch (err) {
...
async function checkProgressAchievements(tgId) {
  try {
    const u = await pool.query('SELECT shhhtoshi, og_pass, og_pass_expires_at FROM users WHERE telegram_id = $1', [tgId]);
    if (!u.rows.length) return;
    const bal = Number(u.rows[0].shhhtoshi) || 0;
    await checkAndGrantAchievements(tgId, 'shhhtoshi', bal);
    let og = !!u.rows[0].og_pass;
    if (og && u.rows[0].og_pass_expires_at && new Date(u.rows[0].og_pass_expires_at) < new Date()) og = false;
    if (og) await checkAndGrantAchievements(tgId, 'og_pass', 1);
    const cards = await pool.query('SELECT COUNT(*)::int AS c, COALESCE(MAX(card_level),0)::int AS max_lv FROM user_income_cards WHERE telegram_id = $1', [tgId]);
    await checkAndGrantAchievements(tgId, 'cards_owned', cards.rows[0]?.c || 0);
    await checkAndGrantAchievements(tgId, 'card_level', cards.rows[0]?.max_lv || 0);
  } catch (e) {
    console.warn('checkProgressAchievements', e.message);
  }
}
...
async function checkAndGrantAchievements(tgId, triggerType, value) {
  try {
    ...
    for (const rule of rules.rows) {
      ...
      if (rule.reward_sp) {
        const achReward = await withOgTaskBonus(tgId, rule.reward_sp);
        await pool.query(`UPDATE users SET shhhtoshi = shhhtoshi + $1 WHERE telegram_id = $2`, [achReward, tgId]);
        await pool.query(
          `INSERT INTO balance_ledger (telegram_id, delta, type, note) VALUES ($1, $2, 'achievement', $3)`,
          [tgId, achReward, `Achievement: ${rule.title}${achReward !== rule.reward_sp ? ' (OG +20%)' : ''}`]
        ).catch(() => {});
      }
...
app.post('/api/missions/claim', taskLimiter, authMiddleware, async (req, res) => {
  ...
  try {
    ...
    let rewardSp = await withOgTaskBonus(tgId, m.reward_sp || 0);
    if (rewardSp) {
...
app.get(['/api/leaderboard', '/api/leaderboard/all'], authMiddleware, async (req, res) => {
  try {
    ...
    // Ensure profile photos are cached locally (TG file URLs expire)
    const need = r.rows.filter(u => !u.telegram_photo_url || String(u.telegram_photo_url).includes('api.telegram.org')).map(u => u.telegram_id);
    if (need.length) {
      await Promise.race([
        ensurePhotosForUsers(need),
...
app.get('/api/config', authMiddleware, async (req, res) => {
  try {
    ...
    res.json({
      ...
      shopBoosts: s.shop_boosts || [],
      betaTesting: !!s.beta_testing,
      ogPassGramPrice: parseFloat(s.og_pass_gram_price) || 1,
      spToShhht: s.sp_to_shhht ?? 100,
      withdrawalMin: s.withdrawal_min ?? 10000,
      withdrawalCooldownHours: s.withdrawal_cooldown_hours ?? 0,
      withdrawalCooldownEnabled: !!s.withdrawal_cooldown_enabled,
      withdrawalOpensAt: s.withdrawal_opens_at || null,
      withdrawalScheduleEnabled: !!s.withdrawal_schedule_enabled,
      tasks: visibleTasks,
...
app.post('/api/settings', authMiddleware, adminMiddleware, async (req, res) => {
  const { spin_price, sp_to_shhht, tasks, home_boost_tasks, stake_task, tap_boost_price, tap_boost_amount, withdrawal_min, withdrawal_cooldown_hours, withdrawal_cooldown_enabled, withdrawal_opens_at, withdrawal_schedule_enabled, mysteryBoxes, adsTasks, ads_tasks, og_pass_stars_price, og_pass_gram_price, spin_stars_price, shop_boosts, beta_testing } = req.body;
  try {
    ...
    if (shop_boosts !== undefined) add('shop_boosts', JSON.stringify(shop_boosts));
    if (beta_testing !== undefined) add('beta_testing', !!beta_testing);
    ...
    add('withdrawal_cooldown_hours', withdrawal_cooldown_hours);
    add('withdrawal_cooldown_enabled', withdrawal_cooldown_enabled);
    add('withdrawal_opens_at', withdrawal_opens_at);
    add('withdrawal_schedule_enabled', withdrawal_schedule_enabled);
...
app.post(['/api/withdrawal', '/api/withdrawal/request'], authMiddleware, async (req, res) => {
  ...
  try {
    if (await isBetaTesting()) {
      return res.status(403).json({ error: 'Withdrawals disabled during Beta Testing. No real progress is saved.' });
    }
    const r = await pool.query('SELECT * FROM users WHERE telegram_id = $1', [req.tgUser.id]);
    if (!r.rows.length) return res.status(404).json({ error: 'Not found' });
    const user = r.rows[0];
    ...
    const settings = await pool.query('SELECT sp_to_shhht, withdrawal_min, withdrawal_cooldown_hours, withdrawal_cooldown_enabled, withdrawal_opens_at, withdrawal_schedule_enabled FROM app_settings WHERE id = 1');
    ...
    // Global unlock schedule (admin sets a future date when withdrawals open)
    if (settings.rows[0]?.withdrawal_schedule_enabled && settings.rows[0]?.withdrawal_opens_at) {
      const opens = new Date(settings.rows[0].withdrawal_opens_at);
      if (opens.getTime() > Date.now()) {
        return res.status(403).json({
          error: 'Withdrawals open on ' + opens.toISOString().slice(0, 10),
    ...
    // Per-user cooldown between withdrawals (e.g. 24h = 1/day)
    if (settings.rows[0]?.withdrawal_cooldown_enabled) {
      const hours = Math.max(0, Number(settings.rows[0].withdrawal_cooldown_hours) || 0);
      if (hours > 0) {
        const last = await pool.query(
          `SELECT created_at FROM withdrawals WHERE telegram_id = $1 ORDER BY created_at DESC LIMIT 1`,
          [req.tgUser.id]
        );
        if (last.rows.length) {
          const nextOk = new Date(last.rows[0].created_at).getTime() + hours * 3600 * 1000;
          if (nextOk > Date.now()) {
            return res.status(429).json({
              error: 'Withdrawal cooldown active',
              nextAt: new Date(nextOk).toISOString(),
              cooldownHours: hours
            });
          }
        }
...
app.post('/api/adsgram/claim', authMiddleware, async (req, res) => {
  try {
    ...
    rewardSp = await withOgTaskBonus(tgId, rewardSp);
...
app.use('/photos', express.static(path.join(__dirname, 'public', 'photos'), { maxAge: '7d' }));
...
bot.on('message', async (msg) => {
  try {
    ...
    if (pay.kind === 'incomecard') {
      const cardId = pay.ref_id;
      if (cardId) {
        await pool.query(
          `INSERT INTO user_income_cards (telegram_id, card_id, cycle_started_at, card_level) VALUES ($1,$2,NOW(),1)
           ON CONFLICT (telegram_id, card_id) DO UPDATE SET card_level = LEAST(COALESCE(user_income_cards.card_level,1)+1, 8)`,
          [tgId, cardId]
        );
      }
    } else if (pay.kind === 'ogpass') {
      await pool.query(
...
app.post('/api/stars/invoice', authMiddleware, async (req, res) => {
  ...
  try {
    ...
    if (kind === 'ogpass') {
      // Monthly sub: allow renew anytime (extends expiry)
    ...
    if (kind === 'incomecard') {
      ...
      const c = await pool.query('SELECT * FROM income_cards WHERE id = $1 AND COALESCE(active, TRUE) = TRUE', [cardId]);
      if (!c.rows.length) return res.status(404).json({ error: 'Card not found' });
      const card = c.rows[0];
      const owned = await pool.query('SELECT card_level FROM user_income_cards WHERE telegram_id = $1 AND card_id = $2', [tgId, cardId]);
      const curLevel = owned.rows.length ? (owned.rows[0].card_level || 1) : 0;
      const mult = curLevel === 0 ? 1 : Math.pow(1.5, curLevel);
      const baseStars = Number(card.price_stars) || 0;
      const amount = Math.max(1, Math.ceil(baseStars * mult));
      const payload = `incomecard:${tgId}:${cardId}:${Date.now()}`;
      await pool.query(
        `INSERT INTO stars_payments (telegram_id, payload, kind, amount_stars, ref_id) VALUES ($1,$2,'incomecard',$3,$4)`,
        [tgId, payload, amount, cardId]
      );
      const label = curLevel === 0 ? 'Buy income card' : ('Upgrade card to Lv' + (curLevel + 1));
      const link = await openInvoiceLink(card.title || 'Income Card', label, payload, amount);
      return res.json({ invoiceLink: link, amount, cardId, level: curLevel });
    }
...
app.post('/api/stars/confirm', authMiddleware, async (req, res) => {
  ...
  try {
    ...
    if (kind === 'incomecard') {
      const cardId = req.body.cardId || req.body.card_id;
      const paid = await pool.query(
        `SELECT * FROM stars_payments WHERE telegram_id = $1 AND kind = 'incomecard' AND status = 'paid' AND ref_id = $2 ORDER BY id DESC LIMIT 1`,
        [tgId, String(cardId || '')]
      );
      // Webhook already applied level — just return user + owned
      const u = await pool.query('SELECT * FROM users WHERE telegram_id = $1', [tgId]);
      const owned = await pool.query('SELECT * FROM user_income_cards WHERE telegram_id = $1', [tgId]);
      return res.json({ ok: true, user: u.rows[0], owned: owned.rows, paid: paid.rows.length > 0 });
    }
    if (kind === 'shopboost') {
      const boostId = req.body.boost_id || req.body.boostId;
      const paid = await pool.query(
        `SELECT * FROM stars_payments WHERE telegram_id = $1 AND kind = 'shopboost' AND status = 'paid' AND ref_id = $2 ORDER BY id DESC LIMIT 1`,
        [tgId, String(boostId || '')]
      );
      if (!paid.rows.length) return res.status(402).json({ error: 'Payment not confirmed yet' });
      // Apply boost via same claim path flags
      const claimBody = { boost_id: boostId, payment_ok: true };
      // inline apply
      const s = await pool.query('SELECT shop_boosts, home_boost_tasks FROM app_settings WHERE id = 1');
      const list = s.rows[0]?.shop_boosts || s.rows[0]?.home_boost_tasks || [];
      const boosts = typeof list === 'string' ? JSON.parse(list) : list;
      const bt = (boosts || []).find((b) => b.id === boostId);
      if (!bt) return res.status(404).json({ error: 'Boost not found' });
      const u0 = await pool.query('SELECT tap_limit_boosted, tap_per_hit FROM users WHERE telegram_id = $1', [tgId]);
      let boosted = u0.rows[0]?.tap_limit_boosted || {};
      if (typeof boosted === 'string') try { boosted = JSON.parse(boosted); } catch { boosted = {}; }
      const count = Number(boosted[boostId] || 0);
      const maxC = bt.maxCount || 1;
      if (count >= maxC) return res.status(409).json({ error: 'Already maxed' });
      boosted[boostId] = count + 1;
      if (bt.kind === 'tap_power' || bt.type === 'tap_power') {
        const add = Number(bt.boost || bt.amount || 1);
        await pool.query('UPDATE users SET tap_per_hit = COALESCE(tap_per_hit,1) + $1, tap_limit_boosted = $2::jsonb WHERE telegram_id = $3', [add, JSON.stringify(boosted), tgId]);
      } else {
        await pool.query('UPDATE users SET tap_limit_boosted = $1::jsonb WHERE telegram_id = $2', [JSON.stringify(boosted), tgId]);
      }
      const u = await pool.query('SELECT * FROM users WHERE telegram_id = $1', [tgId]);
...
app.post('/api/ogpass/prepare', authMiddleware, async (req, res) => {
  ...
  try {
    ...
    // Monthly sub: allow renew anytime
...
app.post('/api/ogpass/buy', authMiddleware, async (req, res) => {
  ...
  try {
    ...
    await pool.query(
      `UPDATE users SET og_pass = TRUE, og_pass_bought_at = COALESCE(og_pass_bought_at, NOW()),
       og_pass_expires_at = GREATEST(COALESCE(og_pass_expires_at, NOW()), NOW()) + INTERVAL '30 days',
       tap_per_hit = CASE WHEN COALESCE(og_tap_bonus_applied, FALSE) THEN COALESCE(tap_per_hit,1)
                            ELSE COALESCE(tap_per_hit,1) + 2 END,
         og_tap_bonus_applied = TRUE,
       updated_at = NOW() WHERE telegram_id = $1`,
    ...
    const u = await pool.query('SELECT * FROM users WHERE telegram_id = $1', [tgId]);
    try { await checkProgressAchievements(tgId); } catch (_) {}
...
// ── Income Cards (Shop → Cards) ──────────────────────────────────────────────
app.get('/api/income-cards', authMiddleware, async (req, res) => {
  try {
    const cards = await pool.query('SELECT * FROM income_cards WHERE active = TRUE ORDER BY sort_order, title');
    const owned = await pool.query('SELECT * FROM user_income_cards WHERE telegram_id = $1', [req.tgUser.id]);
    const og = await pool.query('SELECT og_pass FROM users WHERE telegram_id = $1', [req.tgUser.id]);
    const isOg = !!og.rows[0]?.og_pass;
    res.json({ cards: cards.rows, owned: owned.rows, ogPass: isOg });
  } catch (e) {
    res.status(500).json({ error: 'DB error' });
  }
});

app.get('/api/admin/income-cards', authMiddleware, adminMiddleware, async (req, res) => {
  try {
    const r = await pool.query('SELECT * FROM income_cards WHERE COALESCE(active, TRUE) = TRUE ORDER BY sort_order, title');
    res.json({ cards: r.rows });
  } catch (e) { res.status(500).json({ error: 'DB error' }); }
});

app.post('/api/admin/income-cards', authMiddleware, adminMiddleware, async (req, res) => {
  try {
    const { id, title, description, image_url, sp_per_hour, price_gram, price_stars, price_sp, pay_methods, max_level, active, sort_order } = req.body || {};
    const cid = id || ('card_' + Date.now());
    await pool.query(
      `INSERT INTO income_cards (id, title, description, image_url, sp_per_hour, price_gram, price_stars, price_sp, pay_methods, max_level, active, sort_order)
       VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9::jsonb,COALESCE($10,8),COALESCE($11,TRUE),COALESCE($12,0))
       ON CONFLICT (id) DO UPDATE SET
         title=EXCLUDED.title, description=EXCLUDED.description, image_url=EXCLUDED.image_url,
         sp_per_hour=EXCLUDED.sp_per_hour, price_gram=EXCLUDED.price_gram, price_stars=EXCLUDED.price_stars,
         price_sp=EXCLUDED.price_sp, pay_methods=EXCLUDED.pay_methods, max_level=EXCLUDED.max_level,
         active=EXCLUDED.active, sort_order=EXCLUDED.sort_order`,
      [cid, title || 'Card', description || '', image_url || '', sp_per_hour || 100, price_gram || 0, price_stars || 0, price_sp || 0, JSON.stringify(pay_methods || ['gram','stars']), max_level || 8, active, sort_order]
    );
    const r = await pool.query('SELECT * FROM income_cards ORDER BY sort_order, title');
    res.json({ ok: true, cards: r.rows });
  } catch (e) {
    console.error('admin income-cards', e);
    res.status(500).json({ error: 'Failed' });
  }
});

app.delete('/api/admin/income-cards/:id', authMiddleware, adminMiddleware, async (req, res) => {
  try {
    await pool.query('DELETE FROM user_income_cards WHERE card_id = $1', [req.params.id]);
    await pool.query('DELETE FROM income_cards WHERE id = $1', [req.params.id]);
    res.json({ ok: true });
  } catch (e) { res.status(500).json({ error: 'Failed' }); }
});
// Also allow POST delete for clients without DELETE method
app.post('/api/admin/income-cards/:id/delete', authMiddleware, adminMiddleware, async (req, res) => {
  try {
    const id = req.params.id;
    await pool.query('UPDATE income_cards SET active = FALSE WHERE id = $1', [id]);
    await pool.query('DELETE FROM user_income_cards WHERE card_id = $1', [id]);
    await pool.query('DELETE FROM income_cards WHERE id = $1', [id]);
    const r = await pool.query('SELECT * FROM income_cards WHERE active = TRUE ORDER BY sort_order, title');
    res.json({ ok: true, cards: r.rows });
  } catch (e) {
    console.error('delete income card', e);
    res.status(500).json({ error: 'Failed: ' + e.message });
  }
});

// Claim accumulated Sp from owned cards (up to 6h per cycle)
app.post('/api/income-cards/claim', authMiddleware, async (req, res) => {
  const tgId = req.tgUser.id;
  try {
    const isOg = await userHasActiveOg(tgId);
    const mult = isOg ? 2 : 1;
    const owned = await pool.query(
      `SELECT uic.*, ic.sp_per_hour, ic.title FROM user_income_cards uic
       JOIN income_cards ic ON ic.id = uic.card_id
       WHERE uic.telegram_id = $1 AND COALESCE(uic.active, TRUE) = TRUE AND COALESCE(ic.active, TRUE) = TRUE`,
      [tgId]
    );
    let total = 0;
    const now = Date.now();
    const MIN_MS = 60 * 1000; // at least 1 minute
    for (const row of owned.rows) {
      const start = new Date(row.cycle_started_at).getTime();
      const elapsedMs = Math.max(0, now - start);
      if (elapsedMs < MIN_MS) continue;
      const elapsedH = Math.min(6, elapsedMs / 3600000);
      const lvl = row.card_level || 1;
      const earned = Math.floor(elapsedH * (row.sp_per_hour || 0) * lvl * mult);
      if (earned <= 0) continue;
      total += earned;
      // Reset timer from now after any claim
      await pool.query(
        `UPDATE user_income_cards SET cycle_started_at = NOW(), last_claim_at = NOW() WHERE id = $1`,
        [row.id]
      );
    }
    if (total <= 0) {
      return res.json({ ok: false, error: 'Nothing to claim yet — wait at least 1 minute', reward: 0 });
    }
    await pool.query('UPDATE users SET shhhtoshi = shhhtoshi + $1 WHERE telegram_id = $2', [total, tgId]);
    await pool.query(
      `INSERT INTO balance_ledger (telegram_id, delta, type, note) VALUES ($1, $2, 'income_card', $3)`,
      [tgId, total, 'Income cards claim']
    ).catch(() => {});
    const u = await pool.query('SELECT * FROM users WHERE telegram_id = $1', [tgId]);
    res.json({ ok: true, reward: total, total, user: u.rows[0] });
  } catch (e) {
    console.error('income claim', e);
    res.status(500).json({ error: 'Failed' });
  }
});
...
async function applyIncomeCardPurchase(tgId, card_id) {
  const c = await pool.query('SELECT * FROM income_cards WHERE id = $1 AND COALESCE(active, TRUE) = TRUE', [card_id]);
  if (!c.rows.length) throw new Error('Card not found');
  const card = c.rows[0];
  const exist = await pool.query('SELECT * FROM user_income_cards WHERE telegram_id = $1 AND card_id = $2', [tgId, card_id]);
  const curLevel = exist.rows.length ? (exist.rows[0].card_level || 1) : 0;
  const maxL = card.max_level || 8;
  if (curLevel >= maxL) throw new Error('Max level reached');
  if (exist.rows.length) {
    await pool.query(
      'UPDATE user_income_cards SET card_level = COALESCE(card_level,1) + 1 WHERE telegram_id = $1 AND card_id = $2',
      [tgId, card_id]
    );
  } else {
    await pool.query(
      `INSERT INTO user_income_cards (telegram_id, card_id, cycle_started_at, card_level, active)
       VALUES ($1,$2,NOW(),1,TRUE)
       ON CONFLICT (telegram_id, card_id) DO UPDATE SET card_level = 1, active = TRUE, cycle_started_at = NOW()`,
      [tgId, card_id]
    );
  }
  try { await checkProgressAchievements(tgId); } catch (_) {}
  const owned = await pool.query('SELECT * FROM user_income_cards WHERE telegram_id = $1', [tgId]);
  const u = await pool.query('SELECT * FROM users WHERE telegram_id = $1', [tgId]);
  return { owned: owned.rows, user: u.rows[0], upgraded: curLevel > 0, nextLevel: curLevel + 1 };
}


app.post('/api/income-cards/buy', authMiddleware, async (req, res) => {
  const tgId = req.tgUser.id;
  const { card_id, method } = req.body || {};
  ...
  try {
    ...
    const curLevel = exist.rows.length ? (exist.rows[0].card_level || 1) : 0;
    const maxL = card.max_level || 8;
    if (curLevel >= maxL) return res.status(409).json({ error: 'Max level reached' });

    // Pricing: first buy = base; each upgrade = base * 1.5^currentLevel
    const mult = curLevel === 0 ? 1 : Math.pow(1.5, curLevel);
    const baseSp = Number(card.price_sp) || 0;
    const baseGram = Number(card.price_gram) || 0;
    const baseStars = Number(card.price_stars) || 0;
    const costSp = Math.ceil(baseSp * mult);
    const costGram = Math.round(baseGram * mult * 100) / 100;
    const costStars = Math.ceil(baseStars * mult);

    if (m === 'stars') {
      if (costStars <= 0) return res.status(400).json({ error: 'Stars price not configured' });
      return res.json({ ok: false, needsPayment: true, method: 'stars', amount: costStars, card_id });
    } else if (m === 'gram') {
      if (costGram <= 0) return res.status(400).json({ error: 'Gram price not configured' });
      if (!req.body?.payment_ok) {
        return res.json({ ok: false, needsPayment: true, method: 'gram', amount: costGram, card_id });
      }
      const comment = req.body?.tx_comment || (`Card ${card_id} L${curLevel + 1} ID:${tgId}`);
      try {
        const ver = await verifyTonPayment(comment, costGram);
        if (!ver.verified) console.warn('income-card gram unverified', ver.reason);
      } catch (e) { console.warn('income-card gram verify', e.message); }
      // paid — fall through to grant
    } else if (m === 'sp') {
      if (costSp > 0) {
        const pay = await pool.query(
          'UPDATE users SET shhhtoshi = shhhtoshi - $1 WHERE telegram_id = $2 AND shhhtoshi >= $1 RETURNING shhhtoshi',
          [costSp, tgId]
        );
        if (!pay.rows.length) return res.status(402).json({ error: 'Not enough Sp (need ' + costSp + ')' });
      } else if (curLevel > 0) {
        return res.status(400).json({ error: 'This upgrade requires Gram, Stars, or Ad' });
      }
    } else if (m === 'ad') {
      if (!req.body?.ad_completed) {
        return res.status(402).json({ error: 'Watch the ad first', needsAd: true });
      }
      await pool.query(
        `INSERT INTO task_completions (telegram_id, task_id, task_type, price) VALUES ($1,$2,'ad',0)`,
        [tgId, 'income_card_' + card_id + '_lv' + (curLevel + 1)]
      ).catch(() => {});
    } else {
    ...
    res.json({
      ...
      user: u.rows[0],
      nextCost: {
        sp: Math.ceil(baseSp * Math.pow(1.5, curLevel + 1)),
        gram: Math.round(baseGram * Math.pow(1.5, curLevel + 1) * 100) / 100,
        stars: Math.ceil(baseStars * Math.pow(1.5, curLevel + 1))
      }
    });
  } catch (e) {
    console.error('income buy', e);
    res.status(500).json({ error: 'Failed: ' + (e.message || e) });
  }
});

// Claim uses level multiplier: sp_per_hour * level




// Shop boosts claim (limit / channel / ad)
app.post('/api/shop-boosts/claim', authMiddleware, async (req, res) => {
  const tgId = req.tgUser.id;
  const { boost_id, ad_completed, payment_ok, tx_comment } = req.body || {};
  if (!boost_id) return res.status(400).json({ error: 'Missing boost_id' });
  try {
    const s = await pool.query('SELECT shop_boosts, home_boost_tasks FROM app_settings WHERE id = 1');
    const list = s.rows[0]?.shop_boosts || s.rows[0]?.home_boost_tasks || [];
    const boosts = typeof list === 'string' ? JSON.parse(list) : list;
    const bt = (boosts || []).find((b) => b.id === boost_id);
    if (!bt) return res.status(404).json({ error: 'Boost not found' });
    const u = await pool.query('SELECT tap_limit_boosted, tap_per_hit, shhhtoshi FROM users WHERE telegram_id = $1', [tgId]);
    let boosted = u.rows[0]?.tap_limit_boosted || {};
    if (typeof boosted === 'string') try { boosted = JSON.parse(boosted); } catch { boosted = {}; }
    const key = boost_id;
    const count = Number(boosted[key] || 0);
    const maxC = bt.maxCount || 1;
    if (count >= maxC) return res.status(409).json({ error: 'Already maxed' });

    const reqType = bt.requirement || (bt.type === 'ad' || /ad/i.test(bt.id || '') ? 'ad'
      : (/channel|telegram/i.test(bt.id || '') || /channel|telegram/i.test(bt.title || '') ? 'channel' : 'claim'));

    // Channel / group membership verification (bot must be admin in the chat)
    if (reqType === 'channel') {
      let chatId = bt.channelId || bt.chatId || bt.telegram_chat_id || null;
      const url = bt.channelUrl || bt.url || bt.link || '';
      if (!chatId && url) {
        // Accept @username, t.me/username, or numeric -100... ids pasted in URL field
        const mUser = String(url).match(/(?:t\.me\/|@)([A-Za-z0-9_]+)/i);
        const mNum = String(url).match(/(-?\d{6,})/);
        if (mNum) chatId = mNum[1];
        else if (mUser) chatId = '@' + mUser[1];
      }
      const v = await verifyTelegramMembership(chatId, tgId);
      if (!v.ok) return res.status(403).json({ error: v.error || 'Join the channel first' });
    }
    ...
    if (reqType === 'stars') {
      return res.json({ ok: false, needsPayment: true, method: 'stars', amount: Number(bt.starsPrice) || 0, boost_id });
    }

    if (reqType === 'gram') {
      if (!payment_ok) {
        return res.json({ ok: false, needsPayment: true, method: 'gram', amount: Number(bt.gramPrice) || 0, boost_id });
      }
      // Optional on-chain check when client sends tx_comment after wallet send
      if (tx_comment && Number(bt.gramPrice) > 0) {
        try {
          const ver = await verifyTonPayment(tx_comment, Number(bt.gramPrice));
          if (!ver.verified) {
            console.warn('shop-boost gram payment unverified', ver.reason);
          }
        } catch (e) { console.warn('shop-boost gram verify', e.message); }
      }
    }

    if (reqType === 'sp') {
      const cost = Number(bt.spPrice || bt.price || 0);
      if (cost > 0) {
        const pay = await pool.query(
          'UPDATE users SET shhhtoshi = shhhtoshi - $1 WHERE telegram_id = $2 AND shhhtoshi >= $1 RETURNING shhhtoshi',
          [cost, tgId]
        );
        if (!pay.rows.length) return res.status(402).json({ error: 'Not enough Sp (need ' + cost + ')' });
      }
    }
    ...
    if (bt.kind === 'tap_power' || bt.type === 'tap_power') {
      const add = Number(bt.boost || bt.amount || 1);
      await pool.query(
        'UPDATE users SET tap_per_hit = COALESCE(tap_per_hit,1) + $1, tap_limit_boosted = $2::jsonb WHERE telegram_id = $3',
        [add, JSON.stringify(boosted), tgId]
      );
    } else {
      await pool.query('UPDATE users SET tap_limit_boosted = $1::jsonb WHERE telegram_id = $2', [JSON.stringify(boosted), tgId]);
    }
    const fresh = await pool.query('SELECT * FROM users WHERE telegram_id = $1', [tgId]);
    res.json({ ok: true, user: fresh.rows[0] });
  } catch (e) {
    console.error('shop-boost claim', e);
    res.status(500).json({ error: 'Failed' });
  }
});


// ── Beta feedback → admin(s) via Telegram bot ────────────────────────────────
app.post('/api/feedback', authMiddleware, async (req, res) => {
  try {
    const { rating, text } = req.body || {};
    const stars = Math.max(0, Math.min(5, parseInt(rating, 10)));
    if (Number.isNaN(stars)) return res.status(400).json({ error: 'Invalid rating' });
    const body = String(text || '').trim();
    const words = body ? body.split(/\s+/).filter(Boolean) : [];
    if (words.length > 150) return res.status(400).json({ error: 'Max 150 words' });
    if (!body && stars === 0) return res.status(400).json({ error: 'Add a rating or a short message' });

    const u = req.tgUser || {};
    const name = [u.first_name, u.last_name].filter(Boolean).join(' ') || 'User';
    const uname = u.username ? '@' + u.username : 'no username';
    const starLine = '⭐'.repeat(stars) + (stars < 5 ? '☆'.repeat(5 - stars) : '');
    const msg =
      '🧪 <b>Beta Feedback</b>\n\n' +
      '👤 <b>' + name.replace(/</g, '') + '</b> (' + uname + ')\n' +
      '🆔 <code>' + u.id + '</code>\n' +
      '⭐ Rating: <b>' + stars + '/5</b> ' + starLine + '\n\n' +
      (body ? '💬 ' + body.replace(/</g, '').slice(0, 2000) : '💬 (no text)');

    const adminIds = await getAdminIds();
    let sent = 0;
    for (const aid of adminIds) {
      try {
        await bot.sendMessage(aid, msg, { parse_mode: 'HTML' });
        sent++;
      } catch (e) {
        console.warn('feedback notify admin', aid, e.message);
      }
    }
