...
app.post('/api/stars/invoice', authMiddleware, async (req, res) => {
  ...
  try {
    ...
    if (kind === 'incomecard') {
      ...
      const c = await pool.query('SELECT * FROM income_cards WHERE id = $1 AND active = TRUE', [cardId]);
      if (!c.rows.length) return res.status(404).json({ error: 'Card not found' });
      const card = c.rows[0];
      const amount = parseInt(card.price_stars, 10) || parseInt(req.body.amount, 10) || 50;
      ...
      const link = await openInvoiceLink(card.title || 'Income Card', 'Buy income card', payload, amount);
      return res.json({ invoiceLink: link, amount, cardId });
    }
  ...
  try {
    ...
    if (m === 'sp') {
      if (costSp > 0) {
        ...
        // free upgrades not allowed if base is 0 for paid methods — require gram/stars/ad
        return res.status(400).json({ error: 'This upgrade requires Gram, Stars, or Ad' });
      }
    } else if (m === 'ad') {
      await pool.query(
        `INSERT INTO task_completions (telegram_id, task_id, task_type, reward) VALUES ($1,$2,'ad',0)`,
        [tgId, 'income_card_' + card_id + '_lv' + (curLevel + 1)]
      ).catch(() => {});
    } else if (m === 'gram') {
      // Client opens wallet transfer; if base gram is 0 only first buy free
      if (curLevel > 0 && costGram <= 0 && costSp <= 0 && costStars <= 0) {
        return res.status(400).json({ error: 'Upgrade price not configured' });
      }
    } else if (m === 'stars') {
      if (curLevel > 0 && costStars <= 0) {
        return res.status(400).json({ error: 'Stars upgrade price not configured' });
      }
    }
