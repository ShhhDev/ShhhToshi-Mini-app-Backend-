...
app.post('/api/admin/income-cards', authMiddleware, adminMiddleware, async (req, res) => {
  try {
    const { id, title, description, image_url, sp_per_hour, price_gram, price_stars, active, sort_order } = req.body || {};
    const cid = id || ('card_' + Date.now());
    await pool.query(
      `INSERT INTO income_cards (id, title, description, image_url, sp_per_hour, price_gram, price_stars, active, sort_order)
      ...
      [cid, title || 'Card', description || '', image_url || '', sp_per_hour || 100, price_gram || 0, price_stars || 0, active, sort_order]
    );
    const r = await pool.query('SELECT * FROM income_cards ORDER BY sort_order, title');
...
app.post('/api/income-cards/claim', authMiddleware, async (req, res) => {
  ...
  try {
    ...
    for (const row of owned.rows) {
      ...
      const earned = Math.floor(6 * row.sp_per_hour * mult);
      total += earned;
      await pool.query(
        `UPDATE user_income_cards SET cycle_started_at = NOW(), last_claim_at = NOW() WHERE id = $1`,
        [row.id]
...
app.post('/api/income-cards/buy', authMiddleware, async (req, res) => {
  ...
  try {
    ...
    const card = c.rows[0];
    const exist = await pool.query('SELECT 1 FROM user_income_cards WHERE telegram_id = $1 AND card_id = $2', [tgId, card_id]);
    if (exist.rows.length) return res.status(409).json({ error: 'Already owned' });
    // Stars / gram payment verified separately by existing flows; for now grant after client confirms stars or gram
    if (method === 'free_admin') {
      // admin test only blocked
    }
    await pool.query(
      `INSERT INTO user_income_cards (telegram_id, card_id, cycle_started_at) VALUES ($1,$2,NOW())
    ...
    res.json({ ok: true, owned: owned.rows });
  } catch (e) {
