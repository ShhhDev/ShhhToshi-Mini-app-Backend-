...
async function withOgTaskBonus(tgId, amount) {
  ...
  if (n <= 0) return 0;
  try {
    const r = await pool.query('SELECT og_pass FROM users WHERE telegram_id = $1', [tgId]);
    if (r.rows[0]?.og_pass) n = Math.floor(n * 1.2);
  } catch (_) {}
  return n;
}
...
app.post('/api/income-cards/claim', authMiddleware, async (req, res) => {
  ...
  try {
    ...
    for (const row of owned.rows) {
      ...
      const elapsedH = Math.min(6, Math.max(0, (now - start) / 3600000));
      const earned = Math.floor(elapsedH * row.sp_per_hour * mult);
      if (earned <= 0) continue;
      total += earned;
      // Reset cycle only if 6h fully elapsed (must claim to continue)
      if (elapsedH >= 6) {
        await pool.query(
        ...
        // Partial claim not allowed — wait full 6h
        continue;
      }
    }
    // Recalculate only full cycles
    total = 0;
    for (const row of owned.rows) {
      const start = new Date(row.cycle_started_at).getTime();
      const elapsedH = (now - start) / 3600000;
      if (elapsedH < 6) continue;
      const earned = Math.floor(6 * row.sp_per_hour * (row.card_level || 1) * mult);
      total += earned;
      await pool.query(
        `UPDATE user_income_cards SET cycle_started_at = NOW(), last_claim_at = NOW() WHERE id = $1`,
        [row.id]
    ...
    if (total > 0) {
      ...
      await pool.query(
        ...
        [tgId, total, `Income cards claim (${mult}x)`]
      ).catch(() => {});
    }
    const u = await pool.query('SELECT * FROM users WHERE telegram_id = $1', [tgId]);
    const owned2 = await pool.query('SELECT * FROM user_income_cards WHERE telegram_id = $1', [tgId]);
    res.json({ ok: true, claimed: total, user: u.rows[0], owned: owned2.rows });
  } catch (e) {
...
app.post('/api/income-cards/buy', authMiddleware, async (req, res) => {
  const tgId = req.tgUser.id;
  const { card_id, method, nonce } = req.body || {};
  if (!card_id) return res.status(400).json({ error: 'Missing card_id' });
  try {
    const c = await pool.query('SELECT * FROM income_cards WHERE id = $1 AND active = TRUE', [card_id]);
    if (!c.rows.length) return res.status(404).json({ error: 'Card not found' });
    const card = c.rows[0];
    const methods = Array.isArray(card.pay_methods) ? card.pay_methods : (typeof card.pay_methods === 'string' ? JSON.parse(card.pay_methods || '[]') : ['gram','stars']);
    const m = method || 'sp';
    if (methods.length && !methods.includes(m) && m !== 'ad') {
      // allow if method in list
    }
    ...
    if (exist.rows.length) {
      ...
      if ((cur.card_level || 1) >= maxL) return res.status(409).json({ error: 'Max level reached' });
      // charge for upgrade same as buy
      if (m === 'sp') {
        const cost = card.price_sp || 0;
        if (cost > 0) {
          const pay = await pool.query('UPDATE users SET shhhtoshi = shhhtoshi - $1 WHERE telegram_id = $2 AND shhhtoshi >= $1 RETURNING shhhtoshi', [cost, tgId]);
          if (!pay.rows.length) return res.status(402).json({ error: 'Not enough Sp' });
        }
      } else if (m === 'ad') {
        // client must have watched ad; trust once per call with rate limit via task log
        await pool.query(`INSERT INTO task_completions (telegram_id, task_id, task_type, reward) VALUES ($1,$2,'ad',0)`, [tgId, 'income_card_' + card_id]).catch(()=>{});
      }
      // gram/stars verified client-side before this call
      await pool.query('UPDATE user_income_cards SET card_level = COALESCE(card_level,1) + 1 WHERE telegram_id = $1 AND card_id = $2', [tgId, card_id]);
      const owned = await pool.query('SELECT * FROM user_income_cards WHERE telegram_id = $1', [tgId]);
      const u = await pool.query('SELECT * FROM users WHERE telegram_id = $1', [tgId]);
      return res.json({ ok: true, upgraded: true, owned: owned.rows, user: u.rows[0] });
    }
    if (m === 'sp') {
      const cost = card.price_sp || 0;
      if (cost > 0) {
      ...
      await pool.query(`INSERT INTO task_completions (telegram_id, task_id, task_type, reward) VALUES ($1,$2,'ad',0)`, [tgId, 'income_card_buy_' + card_id]).catch(()=>{});
    }
    await pool.query(
      `INSERT INTO user_income_cards (telegram_id, card_id, cycle_started_at, card_level) VALUES ($1,$2,NOW(),1)
    ...
    const owned = await pool.query('SELECT * FROM user_income_cards WHERE telegram_id = $1', [tgId]);
    const u = await pool.query('SELECT * FROM users WHERE telegram_id = $1', [tgId]);
    res.json({ ok: true, owned: owned.rows, user: u.rows[0] });
  } catch (e) {
