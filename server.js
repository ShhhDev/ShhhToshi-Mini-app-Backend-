...
app.post('/api/income-cards/buy', authMiddleware, async (req, res) => {
  ...
  try {
    ...
    if (m === 'stars') {
      ...
      return res.json({ ok: false, needsPayment: true, method: 'stars', amount: costStars });
    }
    if (m === 'gram') {
      if (costGram <= 0) return res.status(400).json({ error: 'Gram price not configured' });
      return res.json({ ok: false, needsPayment: true, method: 'gram', amount: costGram });
    }
    if (m === 'sp') {
      if (costSp > 0) {
...
app.post('/api/shop-boosts/claim', authMiddleware, async (req, res) => {
  ...
  try {
    ...
    const u = await pool.query('SELECT tap_limit_boosted, tap_per_hit FROM users WHERE telegram_id = $1', [tgId]);
    let boosted = u.rows[0]?.tap_limit_boosted || {};
    if (typeof boosted === 'string') boosted = JSON.parse(boosted);
    const today = new Date().toISOString().split('T')[0];
    const key = boost_id;
    ...
    if (bt.kind === 'tap_power' || bt.type === 'tap_power') {
      ...
      // tap limit boost stored in tap_limit_boosted counts; home uses boost * count
