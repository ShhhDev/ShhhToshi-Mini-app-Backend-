...
migrate().then(() => {
  ...
  setInterval(distributeBlocksDailyReward, 5 * 60 * 1000);   // checks every 5 min, pays out every 24h
  distributeBlocksMonthlyReward();
  setInterval(distributeBlocksMonthlyReward, 60 * 60 * 1000); // checks hourly, pays out once per calendar month
    ...
    if (!isAd && u.referred_by && u.referral_reward_pending) {
      ...
      if (parseInt(countRow.rows[0].count) >= 5) {
        ...
        if (refUser.rows.length) {
          await pool.query(
            'UPDATE users SET shhhtoshi = shhhtoshi + 500, referral_earnings = referral_earnings + 500 WHERE telegram_id = $1',
...
app.post(['/api/spin/prepare', '/api/user/spin/prepare'], spinLimiter, authMiddleware, async (req, res) => {
  ...
  try {
    ...
    // Per-user 30s cooldown: check most recent nonce
    const lastNonce = await pool.query(
      `SELECT created_at FROM spin_nonces WHERE telegram_id = $1 ORDER BY created_at DESC LIMIT 1`, [tgId]
    );
    if (lastNonce.rows.length) {
      const elapsed = Date.now() - new Date(lastNonce.rows[0].created_at).getTime();
      if (elapsed < 30000) {
        return res.status(429).json({ error: `Please wait ${Math.ceil((30000 - elapsed) / 1000)}s before next spin` });
      }
    }
    ...
    // Cryptographically secure random outcome
    const idx = crypto.randomInt(0, WHEEL_VALS.length);
...
app.post('/api/missions/claim', taskLimiter, authMiddleware, async (req, res) => {
  ...
  try {
    ...
    if (m.reward_sp) {
      await pool.query(`UPDATE users SET shhhtoshi = shhhtoshi + $1 WHERE telegram_id = $2`, [m.reward_sp, tgId]);
      await pool.query(
        `INSERT INTO balance_ledger (telegram_id, delta, type, note) VALUES ($1, $2, 'mission', $3)`,
        [tgId, m.reward_sp, `Mission: ${m.title}`]
      ).catch(() => {});
    }
    ...
    const result = await pool.query('SELECT * FROM users WHERE telegram_id = $1', [tgId]);
    res.json({ user: result.rows[0], rewardSp: m.reward_sp, rewardXp: m.reward_xp });
  } catch (err) {
...
app.get(['/api/leaderboard', '/api/leaderboard/all'], authMiddleware, async (req, res) => {
  try {
    const r = await pool.query(
      'SELECT telegram_id, telegram_username, telegram_first_name, telegram_photo_url, shhhtoshi, referrals, tasks_completed FROM users WHERE banned = FALSE ORDER BY shhhtoshi DESC LIMIT 20'
    );
...
app.get('/api/leaderboard/season', authMiddleware, async (req, res) => {
  try {
    const r = await pool.query(
      `SELECT telegram_id, telegram_username, telegram_first_name, telegram_photo_url, season_xp, player_level
...
app.post('/api/settings', authMiddleware, adminMiddleware, async (req, res) => {
  const { spin_price, sp_to_shhht, tasks, home_boost_tasks, stake_task, tap_boost_price, tap_boost_amount, withdrawal_min, mysteryBoxes, adsTasks, ads_tasks } = req.body;
  try {
...
app.post(['/api/withdrawal', '/api/withdrawal/request'], authMiddleware, async (req, res) => {
  ...
  try {
    ...
    sendBotMsg(req.tgUser.id,
      ...
      `🏷 Fee (5%): <b>${Number(feeAmount).toLocaleString()} Sp</b>\n` +
...
async function distributeBlocksDailyReward() { return distributeBlocksReward('daily'); }
async function distributeBlocksMonthlyReward() { return distributeBlocksReward('monthly'); }
